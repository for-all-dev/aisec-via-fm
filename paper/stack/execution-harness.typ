...
#import "../common/figures.typ": harness-flow
...
#figure(
  harness-flow(),
  caption: [A typical request flow through the execution harness. The gateway makes policy decisions on the way in and the way back; the inference server holds the hot path; tool calls divert into a sandbox. The labelled red callouts mark the externally-reachable failure modes addressed later in @sec:tractable-problems.],
) <fig:harness-flow>

Three components make up a typical execution harness, each with its own attack surface.

=== Inference Servers <sec:inference-servers>
...
The security-relevant surface here is large. The server deserializes model weights, often from formats like `SafeTensors` or `GGUF`, and the parsers for the latter have had their own history of memory-corruption bugs @ggml2025gguf (`SafeTensors`' own audit @trailofbits2023safetensors found no critical flaws, but the `GGUF` loader in `llama.cpp` has seen repeated integer-overflow-to-heap-overflow patterns). It accepts user-controlled input (prompts, generation parameters, `LoRA` adapter names) that flows deep into the batching and memory-allocation logic. A malformed request that triggers an out-of-bounds KV-cache write, for instance, could corrupt another user's in-flight completion in a multi-tenant deployment. Formal methods work in this area has two legs: memory-safety of the batching and caching layers, and verified parsers across the several deserialization boundaries the server straddles (weights on the way in, request payloads on the hot path, and structured tool-call outputs on the way back). The second leg is the subject of @sec:verified-input-parsers; the tooling is mature and the gap is targeting, not capability.

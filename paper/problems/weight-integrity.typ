...
#related-layers("execution-harness", "software-framework", "firmware-lowlevel")
...
For the kernel supply chain, the tool is translation validation in the style of Pnueli, Siegel, and Singerman (1998). Each kernel gets a behavioral contract: "given input matrices $X$ and $W$, the output is $X W$ up to floating-point error $epsilon$." A validator checks the compiled binary against this contract without needing source code --- it works on the artifact, which is exactly what you want when the vendor won't ship source. As a stepping stone, differential testing against a formally verified reference implementation (a simple, correct matmul in, say, Fiat Cryptography's style) catches gross violations cheaply. Translation validation catches subtle ones with a proof.

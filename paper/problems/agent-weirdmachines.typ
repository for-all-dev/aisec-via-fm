...
@sec:capability-accumulation takes this seriously as a widget: encode the tool set as a process algebra and model-check reachability of dangerous states. That widget assumes you already know which states count as dangerous and that the composed system is small enough to enumerate. Both assumptions need theoretical backing that does not currently exist.

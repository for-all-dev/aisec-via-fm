...
=== Solution/project Sketch <sec:audit-log-integrity-sketch>

For log integrity, build an `Alloy` model of the storage API and permission system. The specification: no sequence of API calls available to any principal in the serving infrastructure (model, harness, orchestrator) can modify or delete a committed log entry. This is a finite-state check over the permission graph and the storage state machine; `Alloy`'s bounded model checking handles it directly. Separately, specify logging _completeness_ as a `TLA+` liveness property on the serving state machine: every request that reaches forward-pass execution produces exactly one log entry, and that entry is committed before the response is returned to the caller. The completeness property catches suppression; the `Alloy` model catches tampering.
